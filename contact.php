<?php
use PHPMailer\PHPMailer\PHPMailer;
use PHPMailer\PHPMailer\Exception;

require './phpmailer/src/Exception.php';
require './phpmailer/src/PHPMailer.php';
require './phpmailer/src/SMTP.php';

$_SESSION['mailSent'] = false;
if(isset($_POST['submit'])){
     
    $fname = $_POST['fname'];
    $lname = $_POST['lname'];
    $pnumber = $_POST['pnumber'];
    $email = $_POST['email'];
    $inquiry = $_POST['detail'];

    function sendEmail($mail, $from, $to, $body, $subject, $admin="true") {  
        
        // Form SMTP Detail Uncomment and add details
        // $mail->isSMTP();            
        // $mail->Host = "smtp.lolipop.jp"; // Set SMTP host name
        // $mail->SMTPAuth = true;  // Set this to true if SMTP host requires authentication to send email
        // Provide username and password     
        // $mail->Username = "";                 
        // $mail->Password = "";          
        // $mail->SMTPSecure = "ssl"; // If SMTP requires TLS encryption then set it
        // $mail->Port = 465; // Set TCP port to connect to
        // $mail->SMTPDebug  = 2; // To Debug

        $mail->CharSet = 'utf-8';
        $mail->Encoding = 'base64';

        $mail->From = $from;

        $mail->FromName = "HVN Creations";
        $mail->AddAddress($to);
        $mail->IsHTML(true);    // Set email format to HTML
        $mail->Subject = $subject;
        $mail->Body = $body;
        $success = $mail->send();

        $mail->ClearAllRecipients(); 
        $mail->ClearAddresses(); 

        //send the message, check for errors
        if (!$success) {
            $_SESSION['mailSent'] = false;
        } else {
            $_SESSION['mailSent'] = true;
        }
    }

    $from = "info@hvncreations.com";

    //sending mail to admin
    $adminmail = new PHPMailer();

    $adminTo = 'info@hvncreations.com';
    $adminSubject = "Inquiry from Raptors";
    $adminMessage =    
    "Contact form has been send from " . $fname . " " . $lname . " <br/><br/>Details are as below<br/>
    -------------------------------------<br/><br/>
    ■Name<br/>".
    $fname . " " . $lname . "<br/><br/>
    ■Phone Number<br/>".
    $pnumber . "<br/><br/>
    ■Email Address<br/>".
    $email . "<br/><br/>
    ■Inqiury<br/>".
    $inquiry . "<br/><br/>-------------------------------------";
    
    // Send Mail to admin
    sendEmail($adminmail, $from , $adminTo, $adminMessage, $adminSubject); 


    if ($_SESSION['mailSent']) {
        //sending mail to user
        $usermail = new PHPMailer();
        $userSubject = "Your Inquiry has been submitted";
        $userMessage = 
        "Mr/s " . $name . " <br/><br/>
        Thank you for contacting us.<br/>
        We will be contact back you soon.<br/><br/>
        Your inquiry details are as below<br/>-------------------------------------<br/><br/>
        ■Name<br/>".
        $fname . " " . $lname . "<br/><br/>
        ■Phone Number<br/>".
        $pnumber . "<br/><br/>
        ■Email Address<br/>".
        $email . "<br/><br/>
        ■Inqiury<br/>".
        $inquiry . "<br/><br/>
        -------------------------------------<br/><br/>
        HVN CREATIONS <br/>
        Company Details<br/>
        URL：https://hvncreations.com/<br/><br/>";
        $userTo = $email;
        sendEmail($usermail, $from, $userTo, $userMessage, $userSubject, "false");   //Send Mail to User
    }else{
        echo "Mailer Errors: " . $mail->ErrorInfo;
    }

    //send the message, check for errors
    if (!$_SESSION['mailSent']) {
        echo "Mailer Error: " . $mail->ErrorInfo;
    } else {
        header("location: contact.php?sucess=true");
    }
}

?>
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <!-- font-awesome -->
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.13.0/css/all.min.css" rel="stylesheet">

    <!-- animate css -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/animate.css/4.1.1/animate.min.css" />
    <!-- CSS only -->
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css">
    <link rel="stylesheet" href="wickedcss.min.css" />

    <!-- google fonts -->
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Imperial+Script&family=Rajdhani&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="style.css">
    <title>HVNcreations</title>
</head>

<body>
    <header class="header">

        <a href="./index.html"><img src="./img/hvn11.png" class="img-fluid" alt="footer logo" role="presentation"></a>
        <ul class="header__nav">
            <li class="header__list">
                <a class="header__link" href="#">About</a>
            </li>
            <li class="header__list">
                <a class="header__link" href="#">Services</a>
            </li>
            <li class="header__list">
                <a class="header__link" href="#">member</a>
            </li>
            <li class="header__list">
                <a class="header__link" href="#">Contact</a>
            </li>
            <li class="header__list">
                <a class="header__link" href="#">Company</a>
            </li>
        </ul>
    </header>
    <section class="banner">
        <div class="kv-image">
            <!-- <div class="main-text">
                <h1 class="ttl-text ">オフショア開発を通じて絆を深める！<br>悩みなら場所を問わず解決する！</h1>
            </div> -->
        </div>
    </section>
    <section class="about">
        <div class="about-section">
            <div class="inner-container animate__animated animate__lightSpeedInRight wow">
                <h2 class="heading_primary">About</h2>
                <p class="text">
                    生まれはネパールの地方都市で育ちは首都カトマンズです。
                    高校卒業後、およそ10年前に憧れであった日本へ留学すること決意しました。
                    日本に滞在していた姉の家族と同居していたため、暮らし、生活には
                    あまり支障はありませんでしたが最初は大変だったのはやっぱり日本語でした。
                    言葉の壁もあり、日本社会に慣れるまでは少し大変でした。。。
                    (現在は自分の中では凄く馴染みがあるように認識しています。)（笑）
                    留学生でいた自分が愛知県名古屋市内にある日本語学校に通っていて、
                    徐々に日本語の理解も出来るようになり、
                    日本人と会話することがものすごく楽しくなってくるようになりました。
                    その楽しさを比較すると適正にしゃべれないけどとりあえず言葉を
                    発信し周りにいるひとと会話したいという幼稚時代ような気持ちでした。。
                </p>
            </div>
        </div>
    </section>
    <section class="service">
        <h2 class="heading_primary">service</h2>
        <!-- <div class="image"><img src="./img/sakura.png" class="img-fluid" alt=""></div> -->

        <main class="main">
            <div class="container">
                <div class="inner animate__animated animate__lightSpeedInLeft wow" data-wow-delay="600ms">
                    <div class="inner__headings">
                        <h5 class="inner__sub">特徴・強み</h5>
                        <h2 class="inner__head">魅力的なサービスを提供すること、
                            <span class="inner__clr">決して簡単なことではない。</span>
                        </h2>
                    </div>
                </div>
                <div class="row">
                    <div class="col-12 col-sm-6 col-md-4 ">
                        <div class="card animate__animated animate__fadeInUp wow ">
                            <svg class="img-fluid" xmlns="http://www.w3.org/2000/svg" data-name="Layer 1" width="693"
                                height="528" viewBox="0 0 693 528" xmlns:xlink="http://www.w3.org/1999/xlink">
                                <path
                                    d="M946.5,469.5V691.31A22.71737,22.71737,0,0,1,923.81,714H753.67l1.58-2L944.5,472.04Z"
                                    transform="translate(-253.5 -186)" fill="#f2f2f2" />
                                <path
                                    d="M524.3118,391.368H328.943a6.036,6.036,0,0,0,0,12.072H524.3118a6.036,6.036,0,0,0,0-12.072Z"
                                    transform="translate(-253.5 -186)" fill="#e6e6e6" />
                                <path
                                    d="M389.51753,357.52356H328.943a6.036,6.036,0,0,0,0,12.072h60.57458a6.036,6.036,0,0,0,0-12.072Z"
                                    transform="translate(-253.5 -186)" fill="#e6e6e6" />
                                <path
                                    d="M923.81,186H276.19a22.71737,22.71737,0,0,0-22.69,22.69V691.31A22.71737,22.71737,0,0,0,276.19,714H923.81a22.71737,22.71737,0,0,0,22.69-22.69V208.69A22.71737,22.71737,0,0,0,923.81,186ZM944.5,691.31A20.69,20.69,0,0,1,923.81,712H276.19a20.69,20.69,0,0,1-20.69-20.69V343.67554A155.67554,155.67554,0,0,1,411.17554,188H923.81a20.69,20.69,0,0,1,20.69,20.69Z"
                                    transform="translate(-253.5 -186)" fill="#e6e6e6" />
                                <path d="M883.39049,273.26853H316.66a1,1,0,0,1,0-2H883.39049a1,1,0,0,1,0,2Z"
                                    transform="translate(-253.5 -186)" fill="#e6e6e6" />
                                <ellipse cx="91.51486" cy="48.60301" rx="8.45661" ry="8.64507" fill="#e6e6e6" />
                                <ellipse cx="120.72861" cy="48.60301" rx="8.45661" ry="8.64507" fill="#e6e6e6" />
                                <ellipse cx="149.94237" cy="48.60301" rx="8.45661" ry="8.64507" fill="#e6e6e6" />
                                <path
                                    d="M436.75379,241.4358a.9967.9967,0,0,1-.71485-.30078l-5.70605-5.833a.99954.99954,0,0,1,0-1.39844l5.70605-5.83251a1,1,0,1,1,1.42969,1.39843l-5.022,5.1333,5.022,5.13379a1,1,0,0,1-.71484,1.69922Z"
                                    transform="translate(-253.5 -186)" fill="#e6e6e6" />
                                <path
                                    d="M446.34217,241.4358a1,1,0,0,1-.71485-1.69922l5.02149-5.13379-5.02149-5.1333a1,1,0,0,1,1.42969-1.39843l5.70557,5.83251a.99954.99954,0,0,1,0,1.39844l-5.70557,5.833A.9967.9967,0,0,1,446.34217,241.4358Z"
                                    transform="translate(-253.5 -186)" fill="#e6e6e6" />
                                <path
                                    d="M832.852,226.95884h-10.61a2.18935,2.18935,0,0,0-2.19,2.19v10.62a2.18936,2.18936,0,0,0,2.19,2.19h10.61a2.19791,2.19791,0,0,0,2.2-2.19v-10.62A2.1979,2.1979,0,0,0,832.852,226.95884Z"
                                    transform="translate(-253.5 -186)" fill="#e6e6e6" />
                                <path
                                    d="M806.852,226.95884h-10.61a2.18935,2.18935,0,0,0-2.19,2.19v10.62a2.18936,2.18936,0,0,0,2.19,2.19h10.61a2.19791,2.19791,0,0,0,2.2-2.19v-10.62A2.1979,2.1979,0,0,0,806.852,226.95884Z"
                                    transform="translate(-253.5 -186)" fill="#e6e6e6" />
                                <path
                                    d="M857.352,227.45884h-10.61a2.18935,2.18935,0,0,0-2.19,2.19v10.62a2.18936,2.18936,0,0,0,2.19,2.19h10.61a2.19791,2.19791,0,0,0,2.2-2.19v-10.62A2.1979,2.1979,0,0,0,857.352,227.45884Z"
                                    transform="translate(-253.5 -186)" fill="#e6e6e6" />
                                <path
                                    d="M642.4303,231.49881h-84.81a2.67,2.67,0,1,0,0,5.34h84.81a2.67,2.67,0,0,0,0-5.34Z"
                                    transform="translate(-253.5 -186)" fill="#e6e6e6" />
                                <path
                                    d="M415.10743,596.44634l-50.717-1.60323c-7.03419-.22236-14.94447-.211-19.96166,3.42421-5.6943,4.12585-4.83213,11.43535-.10431,16.17646s12.277,7.44976,19.68637,9.652c39.36385,11.7,82.92866,14.07521,125.229,11.42782s83.73-10.13147,124.95545-17.59027c3.91-.70743,8.3414-1.7253,9.93587-4.44631,1.54247-2.63224-.47577-5.80651-3.61075-7.50776s-7.10942-2.27452-10.94329-2.78084a1027.70791,1027.70791,0,0,0-209.50054-5.976"
                                    transform="translate(-253.5 -186)" fill="#f9a826" />
                                <path
                                    d="M506.23,557.44v-12c-8.70812.12331-39.87043-.08886-48,.00027-.605,41.94091-4.31337,33.72515,30.04795,33.88847a5.97483,5.97483,0,0,1,5.95226,5.96665l0,.18a5.965,5.965,0,0,1-5.96511,5.96462H458.23v12h48c.0674-.04016-.04859-11.58721.00015-12,2.02955-34.76507-1.98621-22.26148-31.16579-23.65566a5.0949,5.0949,0,0,1-4.83453-5.09462l0-.1499a5.1001,5.1001,0,0,1,5.09983-5.09981Z"
                                    transform="translate(-253.5 -186)" fill="#3f3d56" />
                                <path
                                    d="M565.23,557.44v-12c-8.70812.12331-39.87043-.08886-48,.00027-.61022,42.30528-4.378,33.57862,30.95215,33.89466a5.1,5.1,0,0,1,5.04814,5.09957c-.00006.63012-.00011,1.27547-.00015,1.90717a5.0988,5.0988,0,0,1-5.09982,5.09833H517.23v12h48c.0674-.04016-.04859-11.58721.00015-12,2.02955-34.76507-1.98621-22.26148-31.16579-23.65566a5.0949,5.0949,0,0,1-4.83453-5.09462l0-.1499a5.1001,5.1001,0,0,1,5.09983-5.09981Z"
                                    transform="translate(-253.5 -186)" fill="#3f3d56" />
                                <path
                                    d="M444.23,557.44v-12h-48c-.034,10.70325.0245,39.0319-.00007,49.99987-9.40881,15.92872,44.04243,4.90065,48.00271,8.00441C444.23,603.44,444.23,591.44,444.23,591.44c-36.5211-.19074-37.43887,6.6459-36.264-26.42981a7.84378,7.84378,0,0,1,7.83373-7.57093C426.358,557.44,444.23,557.44,444.23,557.44Z"
                                    transform="translate(-253.5 -186)" fill="#3f3d56" />
                                <path
                                    d="M539.10743,453.11245l-50.717-1.33642c-7.03419-.18536-14.94447-.17591-19.96166,2.85435-5.6943,3.43922-4.83213,9.53228-.10431,13.48439s12.277,6.21,19.68637,8.04575c39.36385,9.75291,82.92866,11.73282,125.229,9.526s83.73-8.4454,124.95545-14.66291c3.91-.5897,8.3414-1.43818,9.93587-3.70636,1.54247-2.19419-.47577-4.84019-3.61075-6.25832s-7.10942-1.896-10.94329-2.31806a1231.4445,1231.4445,0,0,0-209.50054-4.98149"
                                    transform="translate(-253.5 -186)" fill="#f9a826" />
                                <path
                                    d="M530.22986,400.44c-.81725,18.52643,5.99213,24.3577-17.501,21.99778-1.58724-11.0521,6.46193-26.38959-11.99874-22.49565,3.69817,8.01754-10.3027,68.844,11.99588,57.99455,1.25236-18.27481-7.00207-26.29029,17.50448-23.49444,1.96518,11.297-7.2074,27.92118,11.99857,23.996C538.53142,450.41692,552.53242,389.596,530.22986,400.44Z"
                                    transform="translate(-253.5 -186)" fill="#3f3d56" />
                                <path
                                    d="M653.22986,400.44c1.64714,1.01689-15.09043,18.72384-15.45157,20.1747-.50211-1.23824-16.47736-18.56236-14.5483-20.174-22.22954-10.985-8.37541,50.19481-12.00424,58.002.00411-.00271,12.00411-.00271,12.00411-.00271V420.44177c16.86715,22.74983,13.73491,18.92152,30.00031-1.11688L653.22986,458.44c22.2288,10.98872,8.37573-50.19873,12.00412-58.0027C665.22986,400.44,653.22986,400.44,653.22986,400.44Z"
                                    transform="translate(-253.5 -186)" fill="#3f3d56" />
                                <path
                                    d="M600.22986,400.44h-48c-2.82037,16.82193,6.50508,11.1801,18.00167,11.99963L570.22986,458.44h12v-46C593.67321,411.66182,603.08577,417.18779,600.22986,400.44Z"
                                    transform="translate(-253.5 -186)" fill="#3f3d56" />
                                <path
                                    d="M687.22986,446.44a128.27936,128.27936,0,0,1,0-46h-12c.06918,7.60291-.02139,38.28212-.01434,52.31169a5.71,5.71,0,0,0,5.7058,5.71268c12.08981.016,35.17-.07316,42.30866-.02419L723.22986,446.44Z"
                                    transform="translate(-253.5 -186)" fill="#3f3d56" />
                                <path d="M380.68115,448.44H331.907a9,9,0,0,0,0,18h48.77419a9,9,0,0,0,0-18Z"
                                    transform="translate(-253.5 -186)" fill="#e6e6e6" />
                                <polygon
                                    points="569.392 517.568 559.491 517.567 554.781 479.377 569.394 479.378 569.392 517.568"
                                    fill="#ffb6b6" />
                                <path
                                    d="M823.24065,713.89669l-30.447-.00115v-.385a11.85147,11.85147,0,0,1,11.85083-11.85065h.00073l5.56152-4.21924,10.37659,4.21991,2.65784.00006Z"
                                    transform="translate(-253.5 -186)" fill="#2f2e41" />
                                <polygon
                                    points="604.392 517.568 594.491 517.567 589.781 479.377 604.394 479.378 604.392 517.568"
                                    fill="#ffb6b6" />
                                <path
                                    d="M858.24065,713.89669l-30.447-.00115v-.385a11.85147,11.85147,0,0,1,11.85083-11.85065h.00073l5.56152-4.21924,10.37659,4.21991,2.65784.00006Z"
                                    transform="translate(-253.5 -186)" fill="#2f2e41" />
                                <path
                                    d="M798.23947,495.48018s-9.5262,29.25813-7.76785,33.109S795.23,529.44,792.23,532.44s-5.61884-.189-2.80942,4.90551,7.06559,47.03407,8.93751,49.06428,4.44568,2.08468,2.15879,4.05745S795.23,588.44,798.23,592.44s10.96063,96.27559,10.96063,96.27559h15.55114s3.87318-4.46456,1.68071-7.37008.19247-4.02548,0-5.96549.53674-6.12277.67213-8.03139,2.76354-47.37757-3.55054-73.6431l11.51362-50.01216,5.75679,60.44638s-6.41553,50.70183,0,67.001c0,0-5.17233,4.30709-2.87841,6.80315S842.23,678.44,840.23,680.44s3,9,3,9l17-1,9.62205-94.73447s1.622-9.8797,0-11.57261S868.23,586.44,868.23,580.44v-8s7.25056-49.20472,7.12528-52.10236S863.23,496.44,863.23,496.44Z"
                                    transform="translate(-253.5 -186)" fill="#2f2e41" />
                                <path
                                    d="M848.51935,377.14728l-24.823-1.70728-7.42785,11.712-17.88863,7.27293v57.69454A55.60033,55.60033,0,0,1,801.23,469.44c-.06823,11.49453-3.02253,25.03466-2,28,10,29,65,0,65,0l-1-31,1.3574-70.05937-11.23893-8.96384Z"
                                    transform="translate(-253.5 -186)" fill="#e6e6e6" />
                                <path
                                    d="M797.23253,508.00462,795.521,489.76337l-10.55686-.47618,2.49569,19.2214a6.13324,6.13324,0,1,0,9.77265-.504Z"
                                    transform="translate(-253.5 -186)" fill="#ffb6b6" />
                                <path
                                    d="M802.42783,392.88645l-.29266.04951a16.98787,16.98787,0,0,0-12.58455,9.62841c-3.02332,6.5379-12.8398,49.51683-8.53654,55.99808,3.3376,5.02689,2.52673,4.04615.95778,7.838s-4.0957-.25436-2.02478,5.76366,4.34666,5.1593,2.42433,7.48456-3.89421,1.86944-2.02477,5.76366,1.56555,5.20879,1.91895,6.67532.04613,11.78169.04613,11.78169,11.27634-.85538,16.888,1.82668c0,0-3.13114-10.41759-1.36073-12.08558s3.89423-1.86945,2.37818-4.29714-4.95445-2.53012-1.21216-3.74229,2.5369-22.95558,2.5369-22.95558C807.70522,446.47493,805.26991,419.92688,802.42783,392.88645Z"
                                    transform="translate(-253.5 -186)" fill="#e6e6e6" />
                                <path
                                    d="M870.42228,509.30641l.78906-18.30436,10.51939-1.00815-1.52282,19.32283a6.13324,6.13324,0,1,1-9.78563-.01032Z"
                                    transform="translate(-253.5 -186)" fill="#ffb6b6" />
                                <path
                                    d="M859.42612,394.59691l.29478.03468a16.9879,16.9879,0,0,1,13.05426,8.98129c3.34929,6.37706,13.94508,49.56879,11.35067,55.49615-2.41947,5.52769-2.3194,4.16845-.56116,7.87628s4.07765-.46066,2.313,5.65418-4.08085,5.372-2.04366,7.59733,3.98356,1.6706,2.313,5.65417-1.30078,5.28114-1.57975,6.76363.54829,11.769.54829,11.769-11.30514-.28542-16.77439,2.67632c0,0,2.60161-10.56228.74931-12.13883s-3.98358-1.67062-2.59193-4.17169,4.8205-2.77685,1.02182-3.79868-3.6917-22.79838-3.6917-22.79838Z"
                                    transform="translate(-253.5 -186)" fill="#e6e6e6" />
                                <circle cx="583.21719" cy="166.41031" r="19.36219" fill="#ffb6b6" />
                                <path
                                    d="M850.19186,368.98408c2.69286-2.10746,2.95426-6.12449,5.25223-8.65672,3.8421-4.23373,5.64722-8.78866,3.63458-13.80527-1.78015-4.437-1.40178-9.89125-5.23573-12.74732,1.883-.68165,2.32615-2.09722.66469-2.12336-1.12042-.01762-5.29722,2.12375-3.28744-.54383s-1.7946-1.61462-1.543-2.70656,1.8652-1.69671,2.57686-.83115a5.6397,5.6397,0,0,0-8.84591,1.01189c-2.26458-3.39392-4.66838-5.72925-8.13365-3.57547-3.40345,2.11532-3.359,4.73016-7.01886,4.10174-1.72058-.29541-1.88315,2.95523-3.05854,4.246a5.24691,5.24691,0,0,1-4.609,1.96555c-1.7154-.32418-3.02709-2.46193-2.06587-3.91923l-4.043,9.22356-1.62943-1.99866c-2.58913,1.68734-1.73874,6.09027.86747,7.7511s5.993,1.30585,8.99444.56967a35.27556,35.27556,0,0,1,11.05161-.91662c5.6662.80049,9.61278,7.57467,7.51461,12.89861,2.177-2.13776,6.20431-.36368,7.3321,2.47138s.30082,6.02913-.5352,8.96351"
                                    transform="translate(-253.5 -186)" fill="#2f2e41" />
                            </svg>
                            <div class="card__body">
                                <h4 class="card__head">ホームページ制作</h4>
                                <p class="card__content">HTML、CSS、JS、などを使ったランディングページからコーポレートサイトの開発</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-12 col-sm-6 col-md-4 my-5 my-sm-0">
                        <div class="card animate__animated animate__fadeInUp wow" data-wow-delay="250ms">
                            <svg class="img-fluid" xmlns="http://www.w3.org/2000/svg" data-name="Layer 1" width="752"
                                height="613.12089" viewBox="0 0 752 613.12089"
                                xmlns:xlink="http://www.w3.org/1999/xlink">
                                <title>preservation</title>
                                <path
                                    d="M936.06709,644.81582c13.15168,46.82-113.43889,153.7908-336.16167,94.07509-179.32334-48.07958-336.16167-68.44335-336.16167-94.07509s150.50471,1.25433,336.16167,1.25433S929.13548,620.13917,936.06709,644.81582Z"
                                    transform="translate(-224 -143.43955)" fill="#3f3d56" />
                                <path
                                    d="M936.06709,644.81582c13.15168,46.82-113.43889,153.7908-336.16167,94.07509-179.32334-48.07958-336.16167-68.44335-336.16167-94.07509s150.50471,1.25433,336.16167,1.25433S929.13548,620.13917,936.06709,644.81582Z"
                                    transform="translate(-224 -143.43955)" opacity="0.1" />
                                <path
                                    d="M936.06709,644.81582c13.15168,46.82-113.43889,106.12609-336.16167,46.41038-179.32334-48.07959-336.16167-20.77864-336.16167-46.41038s150.50471-46.41038,336.16167-46.41038S929.13548,620.13917,936.06709,644.81582Z"
                                    transform="translate(-224 -143.43955)" fill="#3f3d56" />
                                <ellipse cx="484" cy="526" rx="26" ry="9" opacity="0.1" />
                                <ellipse cx="590" cy="526" rx="26" ry="9" opacity="0.1" />
                                <ellipse cx="376" cy="486" rx="85" ry="23" opacity="0.1" />
                                <path
                                    d="M976,559.43955v-416H224v416H571v55.60938c-17.33118,2.07513-29,5.9497-29,10.39062,0,6.62738,25.96747,12,58,12s58-5.37262,58-12c0-4.44092-11.66882-8.31549-29-10.39062V559.43955Z"
                                    transform="translate(-224 -143.43955)" fill="#3f3d56" />
                                <rect x="39" y="14.57447" width="674" height="372.85106" fill="#f2f2f2" />
                                <path d="M937,158.43955s-280,29-331,141-343,231-343,231H937Z"
                                    transform="translate(-224 -143.43955)" opacity="0.1" />
                                <path
                                    d="M811.24609,336.43955V364.088H707.75391V338.13h-3V669.43955h3v-59.3125H811.24609v59.3125h3v-333Zm0,30.64844v45.60742H707.75391V367.088ZM707.75391,509.91123V464.30381H811.24609v45.60742Zm103.49218,3v45.60742H707.75391V512.91123ZM707.75391,461.30381v-45.6084H811.24609v45.6084Zm0,145.82324v-45.6084H811.24609v45.6084Z"
                                    transform="translate(-224 -143.43955)" fill="#2f2e41" />
                                <path
                                    d="M675.121,343.02285V347.167s-2.96013,1.18405-2.96013,5.92026-2.36811,22.497-2.36811,22.497,4.14419,3.55216,7.69634,2.36811a14.4625,14.4625,0,0,0,5.32824-2.96014l.592-18.35281-2.36811-4.73621v-8.28837Z"
                                    transform="translate(-224 -143.43955)" fill="#6c7eea" />
                                <path
                                    d="M788.37816,178.43955s-10.29611,1.18406-10.29611,5.92026-8.23688,22.497-8.23688,22.497,14.41455,3.55216,26.76988,2.36811,18.533-2.96013,18.533-2.96013l2.05922-18.35282-8.23689-4.73621Z"
                                    transform="translate(-224 -143.43955)" fill="#6c7eea" />
                                <path
                                    d="M789.382,219.88139v-11.2485s-12.43255-24.8651-2.3681-24.8651,11.2485,22.497,11.2485,22.497v15.9847Z"
                                    transform="translate(-224 -143.43955)" fill="#ffb8b8" />
                                <path
                                    d="M682.81731,337.69461s-6.51229-3.55216-7.69634,0-4.73621,11.2485,0,11.84053,9.47242-3.55216,9.47242-3.55216Z"
                                    transform="translate(-224 -143.43955)" fill="#ffb8b8" />
                                <path
                                    d="M714.86866,361.67553s-3.56489,4.70807,1.60728,50.64,2.19194,79.37638,12.01076,78.918c0,0-2.56277,9.95987,5.78073,10.32728s12.15522.94639,12.80473-1.35475,1.66643-12.94579-.59946-12.84c0,0,.141,3.02118,1.08744-9.13405s-1.73353-69.55756,1.69032-77.28681l13.3894,27.38168s.59946,12.84,3.14744,18.77656,7.94651,40.50377,7.94651,40.50377l-.50846,5.32232s18.2681,2.17488,18.88234-.88156.40268-7.5882-.42314-9.06352-.82582-1.47533.54372-4.567a9.15592,9.15592,0,0,0,.40267-7.5882c-.86108-2.23062-3.15312-51.32472-3.15312-51.32472s-4.29631-59.59769-13.50088-62.19572S714.86866,361.67553,714.86866,361.67553Z"
                                    transform="translate(-224 -143.43955)" fill="#2f2e41" />
                                <path
                                    d="M730.96415,495.65943l-5.54867,10.85621s-9.59986-2.95805-7.30611,5.63968A151.09772,151.09772,0,0,0,745.66,510.4906s3.847,1.33428,4.4965-.96687-2.36428-10.10831-4.63016-10.00252S730.96415,495.65943,730.96415,495.65943Z"
                                    transform="translate(-224 -143.43955)" fill="#2f2e41" />
                                <path
                                    d="M774.63015,490.593s-5.40763,13.87739-3.10649,14.5269,22.83513,2.7186,24.381,3.40337,9.13405,1.08744,9.67777-3.47959-4.05857-5.866-4.05857-5.866-8.37875-1.1227-10.99726-8.56985S774.63015,490.593,774.63015,490.593Z"
                                    transform="translate(-224 -143.43955)" fill="#2f2e41" />
                                <circle cx="499.35674" cy="89.8249" r="15.81446" fill="#ffb8b8" />
                                <path
                                    d="M715.03334,240.75551s4.1617,21.64084,2.497,25.80254,23.30552-1.66468,23.30552-1.66468-4.994-21.64084-4.1617-26.63488S715.03334,240.75551,715.03334,240.75551Z"
                                    transform="translate(-224 -143.43955)" fill="#ffb8b8" />
                                <path
                                    d="M722.87639,213.73132a8.93863,8.93863,0,0,0-3.956.17466,10.315,10.315,0,0,0-2.81141,1.84424,32.90553,32.90553,0,0,1-4.8069,3.18472,11.216,11.216,0,0,0-3.74013,2.62176,7.95819,7.95819,0,0,0-1.05562,1.99817,23.944,23.944,0,0,0-1.25892,13.03151,5.99,5.99,0,0,0,.55443,1.78939c.86508,1.59385,2.77576,2.222,4.3853,3.05749,3.05585,1.5863,5.37231,4.25741,7.8787,6.61805a15.4025,15.4025,0,0,0,3.11291,2.399c3.57571,1.94956,7.9463,1.31355,11.96019.62417a2.15543,2.15543,0,0,0,2.15117-2.176l3.16829-14.25008a23.24887,23.24887,0,0,0,.812-6.48089c-.221-3.45921-2.0938-8.251-4.63253-10.70821C731.67982,214.59626,726.743,214.14067,722.87639,213.73132Z"
                                    transform="translate(-224 -143.43955)" fill="#2f2e41" />
                                <path
                                    d="M711.23457,267.83551s1.18405-8.28836,8.28837-8.88039a58.57015,58.57015,0,0,0,11.84052-2.3681c.592,0,8.28837.592,11.84053,4.14418,0,0,2.3681,3.55216,3.55215,1.18405s1.18406-3.55216,3.55216-2.3681,2.36811-3.55216,6.51229-2.96013,14.80066,27.2332,14.80066,27.2332l-3.55216,4.14419s-4.73621,31.96941,3.55216,45.586,11.84052,28.41726,3.55215,31.37739-66.30694,29.60131-63.93883,13.02458.592-42.62589-1.77608-43.21792-15.39268-47.3621-15.39268-47.3621a18.38855,18.38855,0,0,1,10.65647-13.6166C714.1947,269.61159,711.23457,267.83551,711.23457,267.83551Z"
                                    transform="translate(-224 -143.43955)" fill="#575a88" />
                                <path
                                    d="M753.86046,258.95512l2.96013-2.3681s5.32824,0,5.92026-1.18406,3.55216-1.18405,5.92027-2.3681-.592-2.96013,2.3681-4.14418,2.96013,0,2.96013-2.96014,7.69634-5.92026,8.28837-9.47242S788.79,216.32923,788.79,216.32923l11.2485,3.55216-2.96014,21.31294s0,10.65648-5.32823,17.76079-17.16876,24.8651-23.089,25.45713S753.86046,258.95512,753.86046,258.95512Z"
                                    transform="translate(-224 -143.43955)" fill="#575a88" />
                                <path
                                    d="M697.618,284.41225l-3.55216,2.96013s-1.18405,13.02458-1.18405,13.6166,1.18405,7.69635,0,8.28837-3.55216,12.43255-2.96013,13.02458h0V324.078l-8.28837,13.02457v12.43256l19.53686-12.43256s11.2485-1.18405,9.47242-11.24849S697.618,284.41225,697.618,284.41225Z"
                                    transform="translate(-224 -143.43955)" fill="#575a88" />
                                <circle cx="376" cy="405" r="11" fill="#6c7eea" />
                            </svg>
                            <div class="card__body">
                                <h4 class="card__head">保守・運営</h4>
                                <p class="card__content">webサイトいい状態で持続させるには必要な公開後の保守・運営サポート</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-12 col-sm-6 col-md-4">
                        <div class="card animate__animated animate__fadeInUp wow" data-wow-delay="500ms">
                            <svg class="img-fluid" xmlns="http://www.w3.org/2000/svg" data-name="Layer 1"
                                width="1113.88736" height="612.2441" viewBox="0 0 1113.88736 612.2441"
                                xmlns:xlink="http://www.w3.org/1999/xlink">
                                <title>wordpress</title>
                                <rect x="476.90168" y="96.25" width="452" height="93" fill="none" stroke="#3f3d56"
                                    stroke-miterlimit="10" />
                                <path
                                    d="M250.22415,372.229C263.11336,352.08825,287.517,338.5,315.5,338.5c41.42136,0,75,29.77306,75,66.5,0,23.48168-13.72626,44.12077-34.45252,55.9533"
                                    transform="translate(-43.05632 -143.875)" fill="none" stroke="#3f3d56"
                                    stroke-miterlimit="10" />
                                <rect x="444.90168" y="114.25" width="452" height="93" fill="#00bf39" />
                                <circle cx="159.17106" cy="18.49996" r="7.00009" fill="#00bf39" />
                                <circle cx="183.17106" cy="18.49996" r="7.00009" fill="#00bf39" />
                                <circle cx="207.17106" cy="18.49996" r="7.00009" fill="#00bf39" />
                                <rect x="140.17142" y="0.5" width="840" height="546" fill="none" stroke="#3f3d56"
                                    stroke-miterlimit="10" />
                                <line x1="140.17142" y1="30.83505" x2="980.17142" y2="30.83505" fill="none"
                                    stroke="#3f3d56" stroke-miterlimit="10" />
                                <circle cx="161.17106" cy="15.49996" r="7.00009" fill="none" stroke="#3f3d56"
                                    stroke-miterlimit="10" />
                                <circle cx="185.17106" cy="15.49996" r="7.00009" fill="none" stroke="#3f3d56"
                                    stroke-miterlimit="10" />
                                <circle cx="209.17106" cy="15.49996" r="7.00009" fill="none" stroke="#3f3d56"
                                    stroke-miterlimit="10" />
                                <path
                                    d="M75.27267,348.42356a21.61922,21.61922,0,1,0-26.639-31.90264l8.0969,7.65194L46.32837,320.94a21.53767,21.53767,0,0,0,.42814,16.46985,21.19971,21.19971,0,0,0,3.15322,5.07973A21.6151,21.6151,0,0,0,75.27267,348.42356Z"
                                    transform="translate(-43.05632 -143.875)" fill="#57b894" />
                                <path
                                    d="M183.44639,423.47872c-7.89466-5.53929-7.89618-17.16828-5.93507-26.61092s4.95464-19.74456.50306-28.29983c-6.3977-12.29544-23.21458-13.035-37.01248-14.349a130.15616,130.15616,0,0,1-30.04827-6.5136c-3.8984-1.344-7.80228-2.91353-10.99464-5.52363-4.59737-3.75884-7.29414-9.30238-9.81237-14.68041Q77.56317,300.62766,66.25742,273.18"
                                    transform="translate(-43.05632 -143.875)" fill="none" stroke="#3f3d56"
                                    stroke-miterlimit="10" />
                                <path
                                    d="M73.9308,345.7404a21.61922,21.61922,0,1,0-26.639-31.90265l8.0969,7.65194L44.9865,318.25685a21.53767,21.53767,0,0,0,.42814,16.46985,21.19919,21.19919,0,0,0,3.15322,5.07973A21.61509,21.61509,0,0,0,73.9308,345.7404Z"
                                    transform="translate(-43.05632 -143.875)" fill="none" stroke="#3f3d56"
                                    stroke-miterlimit="10" />
                                <path
                                    d="M47.58089,285.78442a21.594,21.594,0,0,1,1.72991-20.66591l8.61043,6.39795-5.3141-10.29993a21.61655,21.61655,0,1,1-5.02624,24.56789Z"
                                    transform="translate(-43.05632 -143.875)" fill="#57b894" />
                                <path
                                    d="M45.79173,282.20686A21.594,21.594,0,0,1,47.52164,261.541l8.61043,6.398L50.818,257.639a21.61656,21.61656,0,1,1-5.02623,24.56789Z"
                                    transform="translate(-43.05632 -143.875)" fill="none" stroke="#3f3d56"
                                    stroke-miterlimit="10" />
                                <path
                                    d="M138.41288,357.002a21.61661,21.61661,0,0,0-12.83334-40.99231l4.08388,7.915-8.70985-6.47535a.37577.37577,0,0,0-.05415.023A21.61777,21.61777,0,1,0,138.41288,357.002Z"
                                    transform="translate(-43.05632 -143.875)" fill="#57b894" />
                                <path
                                    d="M136.47525,353.49867a21.61661,21.61661,0,0,0-12.83334-40.9923l4.08388,7.915-8.70986-6.47536a.378.378,0,0,0-.05415.02306,21.61777,21.61777,0,1,0,17.51347,39.52961Z"
                                    transform="translate(-43.05632 -143.875)" fill="none" stroke="#3f3d56"
                                    stroke-miterlimit="10" />
                                <path
                                    d="M147.04276,398.48755a21.60757,21.60757,0,1,0-8.71949-15.097l16.98867,3.78678-14.82289,3.56438A21.41544,21.41544,0,0,0,147.04276,398.48755Z"
                                    transform="translate(-43.05632 -143.875)" fill="#57b894" />
                                <path
                                    d="M145.70089,395.80438a21.60757,21.60757,0,1,0-8.71949-15.097l16.98867,3.78678-14.82289,3.56438A21.41544,21.41544,0,0,0,145.70089,395.80438Z"
                                    transform="translate(-43.05632 -143.875)" fill="none" stroke="#3f3d56"
                                    stroke-miterlimit="10" />
                                <path
                                    d="M1084.61287,602.26517a20.81252,20.81252,0,1,0,8.3387-39.13277l-.98562,10.67947-3.7314-9.80018a20.73405,20.73405,0,0,0-12.22123,10.10949,20.40828,20.40828,0,0,0-1.97438,5.40651A20.80854,20.80854,0,0,0,1084.61287,602.26517Z"
                                    transform="translate(-43.05632 -143.875)" fill="#57b894" />
                                <path
                                    d="M1092.0289,728.797c-.49451-9.27109,8.31332-16.18134,16.63109-20.306s17.89981-7.97772,21.73547-16.43263c5.51252-12.15118-3.91862-25.32906-11.121-36.56135a125.3002,125.3002,0,0,1-12.91856-26.63091c-1.29809-3.75147-2.42859-7.64108-2.34816-11.61.11584-5.71565,2.71274-11.052,5.29035-16.15471q12.88015-25.49811,26.95418-50.36938"
                                    transform="translate(-43.05632 -143.875)" fill="none" stroke="#3f3d56"
                                    stroke-miterlimit="10" />
                                <path
                                    d="M1085.84808,599.65459a20.81252,20.81252,0,1,0,8.33869-39.13278l-.98561,10.67948-3.7314-9.80018a20.734,20.734,0,0,0-12.22123,10.10949,20.40859,20.40859,0,0,0-1.97439,5.4065A20.80855,20.80855,0,0,0,1085.84808,599.65459Z"
                                    transform="translate(-43.05632 -143.875)" fill="none" stroke="#3f3d56"
                                    stroke-miterlimit="10" />
                                <path
                                    d="M1115.60841,544.07357a20.78822,20.78822,0,0,1,16.68183-10.96779l.26936,10.32343,4.64475-10.1448a20.81,20.81,0,1,1-21.59594,10.78916Z"
                                    transform="translate(-43.05632 -143.875)" fill="#57b894" />
                                <path
                                    d="M1117.25535,540.59279a20.78824,20.78824,0,0,1,16.68183-10.96779l.26936,10.32344,4.64475-10.14481a20.81,20.81,0,1,1-21.59594,10.78916Z"
                                    transform="translate(-43.05632 -143.875)" fill="none" stroke="#3f3d56"
                                    stroke-miterlimit="10" />
                                <path
                                    d="M1115.62817,655.18935a20.81,20.81,0,0,0,23.42627-34.07564l-3.56911,7.796-.2698-10.44473a.36282.36282,0,0,0-.04963-.02732,20.81113,20.81113,0,1,0-19.53773,36.75172Z"
                                    transform="translate(-43.05632 -143.875)" fill="#57b894" />
                                <path
                                    d="M1117.13066,651.64023a20.81,20.81,0,0,0,23.42627-34.07565l-3.56912,7.796-.26979-10.44474a.36409.36409,0,0,0-.04964-.02732,20.81113,20.81113,0,1,0-19.53772,36.75173Z"
                                    transform="translate(-43.05632 -143.875)" fill="none" stroke="#3f3d56"
                                    stroke-miterlimit="10" />
                                <path
                                    d="M1089.33088,686.374a20.80131,20.80131,0,1,0,6.25521-15.57438l7.225,15.11843-11.50663-9.11037A20.61639,20.61639,0,0,0,1089.33088,686.374Z"
                                    transform="translate(-43.05632 -143.875)" fill="#57b894" />
                                <path
                                    d="M1090.56608,683.76343a20.80131,20.80131,0,1,0,6.25521-15.57438l7.225,15.11842-11.50663-9.11037A20.61655,20.61655,0,0,0,1090.56608,683.76343Z"
                                    transform="translate(-43.05632 -143.875)" fill="none" stroke="#3f3d56"
                                    stroke-miterlimit="10" />
                                <path
                                    d="M158.27267,262.42356a21.61922,21.61922,0,1,0-26.639-31.90264l8.0969,7.65194L129.32837,234.94a21.53767,21.53767,0,0,0,.42814,16.46985,21.19971,21.19971,0,0,0,3.15322,5.07973A21.6151,21.6151,0,0,0,158.27267,262.42356Z"
                                    transform="translate(-43.05632 -143.875)" fill="#57b894" />
                                <path
                                    d="M182.959,256.18177c-4.59737-3.75884-7.29414-9.30238-9.81237-14.68041q-12.58345-26.8737-23.8892-54.32135"
                                    transform="translate(-43.05632 -143.875)" fill="none" stroke="#3f3d56"
                                    stroke-miterlimit="10" />
                                <path
                                    d="M156.9308,259.7404a21.61922,21.61922,0,1,0-26.639-31.90265l8.0969,7.65194-10.40218-3.23284a21.53767,21.53767,0,0,0,.42814,16.46985,21.19919,21.19919,0,0,0,3.15322,5.07973A21.61509,21.61509,0,0,0,156.9308,259.7404Z"
                                    transform="translate(-43.05632 -143.875)" fill="none" stroke="#3f3d56"
                                    stroke-miterlimit="10" />
                                <path
                                    d="M130.58089,199.78442a21.594,21.594,0,0,1,1.72991-20.66591l8.61043,6.398-5.3141-10.29993a21.61655,21.61655,0,1,1-5.02624,24.56789Z"
                                    transform="translate(-43.05632 -143.875)" fill="#57b894" />
                                <path
                                    d="M128.79173,196.20686a21.594,21.594,0,0,1,1.72991-20.66591l8.61043,6.398L133.818,171.639a21.61656,21.61656,0,1,1-5.02623,24.56789Z"
                                    transform="translate(-43.05632 -143.875)" fill="none" stroke="#3f3d56"
                                    stroke-miterlimit="10" />
                                <line x1="403.89774" y1="30.83505" x2="403.89774" y2="547.27591" fill="none"
                                    stroke="#3f3d56" stroke-miterlimit="10" />
                                <rect x="498.40168" y="284.75" width="147" height="20" fill="#f2f2f2" />
                                <rect x="729.40168" y="284.75" width="147" height="20" fill="#f2f2f2" />
                                <rect x="506.40168" y="274.75" width="147" height="20" fill="none" stroke="#3f3d56"
                                    stroke-miterlimit="10" />
                                <rect x="498.40168" y="439.75" width="147" height="20" fill="#f2f2f2" />
                                <rect x="506.40168" y="429.75" width="147" height="20" fill="none" stroke="#3f3d56"
                                    stroke-miterlimit="10" />
                                <rect x="498.90168" y="360.25" width="378" height="20" fill="#f2f2f2" />
                                <rect x="506.90168" y="352.25" width="378" height="20" fill="none" stroke="#3f3d56"
                                    stroke-miterlimit="10" />
                                <rect x="737.40168" y="274.75" width="147" height="20" fill="none" stroke="#3f3d56"
                                    stroke-miterlimit="10" />
                                <path
                                    d="M974.12309,360.06179a20.52086,20.52086,0,0,0,1.20129,3.14327,52.96337,52.96337,0,0,1,.65343,39.86623c3.58806-.22468,7.17109.35557,10.75575.63294s7.295.23158,10.62749-1.17133,6.23339-4.40851,6.71888-8.11679c.65549-5.00687-2.98836-9.37369-6.18738-13.17378-3.25983-3.87234-6.40947-8.14314-7.37416-13.19348-1.06041-5.55146.44893-12.06057-3.058-16.38555a3.38957,3.38957,0,0,0-1.16552-.9944c-1.88372-.86332-6.788-.83559-8.68394.23344C974.69249,352.5479,973.40582,356.88787,974.12309,360.06179Z"
                                    transform="translate(-43.05632 -143.875)" fill="#3f3d56" />
                                <path
                                    d="M1073.42743,722.20289a25.20426,25.20426,0,0,1-1.17772,4.90714s-3.63344,8.36175-18.57731,6.10666c-9.76193-1.47433-9.69655-6.05-8.07825-9.59182a22.85,22.85,0,0,1,2.66512-4.14817c1.45254-1.96285,1.30857-9.45225,1.81893-16.283.506-6.83509,3.5942-7.184,3.5942-7.184l.18322,7.69c5.81876-7.63333,9.88839,1.30857,9.88839,1.30857s8.27437.172,7.26257-4.06964c-1.07779-4.51822-2.68694-11.67244-2.68694-11.67244s4.00424,1.85817,4.72831,3.16674c.72843,1.30857-.14395,18.83031-.79824,20.50093s0,6.1808.94655,7.48937A2.854,2.854,0,0,1,1073.42743,722.20289Z"
                                    transform="translate(-43.05632 -143.875)" fill="#3f3d56" />
                                <path
                                    d="M1070.66634,700.48935a8.14784,8.14784,0,0,1-5.69664,7.76417,10.97309,10.97309,0,0,1-9.58305-1.46123,5.62468,5.62468,0,0,1-2.36854-5.27354l.23114-1.93232.83751-7.04445,1.15154-9.70523s12.38344-6.30731,11.559-3.92571c-.30968.88982.37511,4.894,1.35654,9.56128.69792,3.33686,1.54413,7.00957,2.29,10.124A8.06769,8.06769,0,0,1,1070.66634,700.48935Z"
                                    transform="translate(-43.05632 -143.875)" fill="#fbbebe" />
                                <path d="M1052.25767,699.95721s10.723-12.595,19.50133,1.145l-1.47214,8.80255h-18.02919Z"
                                    transform="translate(-43.05632 -143.875)" opacity="0.1" />
                                <path d="M1052.25767,700.82959s10.723-12.595,19.50133,1.145l-1.47214,8.80255h-18.02919Z"
                                    transform="translate(-43.05632 -143.875)" fill="#3f3d56" />
                                <circle cx="1023.14048" cy="563.63077" r="1.09047" fill="#f2f2f2" />
                                <circle cx="1009.20139" cy="561.88327" r="1.09047" fill="#f2f2f2" />
                                <circle cx="1019.37911" cy="571.02172" r="1.09047" fill="#f2f2f2" />
                                <circle cx="1009.86154" cy="569.74011" r="1.09047" fill="#f2f2f2" />
                                <circle cx="1021.4874" cy="567.55917" r="1.09047" fill="#f2f2f2" />
                                <circle cx="1009.20139" cy="565.81167" r="1.09047" fill="#f2f2f2" />
                                <circle cx="1018.28867" cy="575.07724" r="1.09047" fill="#f2f2f2" />
                                <circle cx="1009.86157" cy="573.66858" r="1.09051" fill="#f2f2f2" />
                                <path
                                    d="M980.43174,735.39764l-14.83046,5.01618s-7.85141-7.85142-5.67047-10.25046c.92909-1.02507,1.7404-4.42734,2.35981-7.99537.20063-1.12538.37948-2.27256.5365-3.363.51473-3.54622.81131-6.52538.81131-6.52538s17.2295-8.28761,14.39427,0a20.40919,20.40919,0,0,0-.81568,7.24947,48.10707,48.10707,0,0,0,.92036,7.25821A69.13914,69.13914,0,0,0,980.43174,735.39764Z"
                                    transform="translate(-43.05632 -143.875)" fill="#fbbebe" />
                                <path
                                    d="M980.43174,735.39764l-14.83046,5.01618s-7.85141-7.85142-5.67047-10.25046c.92909-1.02507,1.7404-4.42734,2.35981-7.99537a15.499,15.499,0,0,1,1.73166,4.833c12.81089,7.03574,13.029,2.83523,13.029,2.83523a7.787,7.787,0,0,1,1.08611-3.049A69.13914,69.13914,0,0,0,980.43174,735.39764Z"
                                    transform="translate(-43.05632 -143.875)" opacity="0.1" />
                                <path
                                    d="M983.70316,746.84762s-2.56479,1.90615-10.0847,2.18095a39.65312,39.65312,0,0,0-5.86591.66064,73.28862,73.28862,0,0,1-24.05818.64958,52.18363,52.18363,0,0,1-6.11829-1.20117c-7.74237-2.12861-1.8538-7.03138-1.8538-7.03138s.3708-.32278,1.012-.807c1.64007-1.23006,5.04232-3.49822,8.53184-4.04783,4.85043-.76333,9.32138-9.37808,9.32138-9.37808l3.05333-5.50908c4.74138-4.0871,6.38146,5.50908,6.38146,5.50908,12.81089,7.03574,13.029,2.83523,13.029,2.83523.711-3.76432,2.01956-3.76432,2.01956-3.76432s3.19289,8.59729,4.34885,10.74336a2.12653,2.12653,0,0,0,.28348.43619C984.4665,738.88715,983.70316,746.84762,983.70316,746.84762Z"
                                    transform="translate(-43.05632 -143.875)" fill="#3f3d56" />
                                <circle cx="920.90925" cy="587.59693" r="1.09047" fill="#f2f2f2" />
                                <circle cx="914.6638" cy="590.65026" r="1.09047" fill="#f2f2f2" />
                                <circle cx="918.02116" cy="589.08641" r="1.09047" fill="#f2f2f2" />
                                <circle cx="911.14952" cy="592.44951" r="1.09047" fill="#f2f2f2" />
                                <path
                                    d="M978.0327,712.27957a20.40919,20.40919,0,0,0-.81568,7.24947c-3.869-1.29983-10.20685-1.073-14.3899-.72409.51473-3.54622.81131-6.52538.81131-6.52538S980.86793,703.992,978.0327,712.27957Z"
                                    transform="translate(-43.05632 -143.875)" opacity="0.1" />
                                <path
                                    d="M1068.15388,688.47232c-5.36076,5.25608-10.486,5.58322-14.06712,4.06966l1.15154-9.70523s12.38344-6.30731,11.559-3.92571C1066.48766,679.80086,1067.17245,683.80508,1068.15388,688.47232Z"
                                    transform="translate(-43.05632 -143.875)" opacity="0.1" />
                                <path
                                    d="M974.108,509.45127s-8.50669,4.79809-10.03335,14.17618-4.14381,21.3733-6.10666,23.55425-8.06951,9.16-5.67047,12.8676,2.83524,16.35712,2.83524,16.35712-.87238,11.34094,1.52666,15.04855,4.3619,27.26187-.87238,34.24091c0,0-1.74476,16.139-1.09047,17.66569s-2.83524,5.45237-1.74476,8.7238,6.32475,63.46563,5.67047,65.86467c0,0,16.57521-2.399,20.93711,1.52666,0,0,.87238-6.54284,0-11.12284s.65428-22.68187,2.61714-27.26187,2.399-22.02758,1.74476-23.77234c0,0,3.48952-15.48475,2.181-18.53807s5.67047-31.18758,5.67047-31.18758,3.48952-6.76094,2.61714-7.63332c0,0,5.67046-3.70762,5.01618-6.54285s5.23428-6.10666,5.23428-6.10666,5.23428,12.8676,6.979,14.83046,1.81321,14.61236,3.95993,17.2295,4.982,16.35712,7.81719,20.06473,14.17617,13.52189,15.92093,24.64473c0,0,10.25047,15.92093,9.81428,20.93711s11.559,14.17617,23.11806-2.83523a15.11356,15.11356,0,0,1-6.10666-12.43141c0-8.50571-9.16-35.76757-9.16-35.76757s-5.23427-7.63333-3.27142-11.34094.2181-14.61236.2181-14.61236l4.1438-6.76094s.65428-6.54285,1.30857-13.3038-3.92571-14.17617-1.74476-17.44759-.87238-19.61656-.87238-19.61656-2.71429-28.17483-6.76094-29.891S974.108,509.45127,974.108,509.45127Z"
                                    transform="translate(-43.05632 -143.875)" fill="#3f3d56" />
                                <path
                                    d="M993.97982,469.97609s-4.47968.51033-9.91895,1.025c-7.86888.74152-17.74859,1.49176-18.92634.71972-1.989-1.30857,0-17.88379,0-17.88379s.68485-18.10188.03056-30.75138c-.53214-10.26356,6.26368-17.80091,8.84158-20.27847.60195-.57577.9727-.87674.9727-.87674l2.18095,1.74476,12.43141,21.15521Z"
                                    transform="translate(-43.05632 -143.875)" fill="#fbbebe" />
                                <path
                                    d="M1042.5888,400.4038l-12.21332,7.41523-20.06473,15.26665s-20.719-10.90475-15.48474-17.2295a9.80613,9.80613,0,0,0,1.91488-4.279c1.57463-7.28437-1.91488-17.74856-1.91488-17.74856s27.29679-36.86241,25.7352-8.28761c-.62809,11.48052,2.80467,17.74857,7.13606,21.14211C1034.14853,401.73418,1042.5888,400.4038,1042.5888,400.4038Z"
                                    transform="translate(-43.05632 -143.875)" fill="#fbbebe" />
                                <path
                                    d="M1042.5888,400.4038l-12.21332,7.41523-20.06473,15.26665s-20.719-10.90475-15.48474-17.2295a9.80613,9.80613,0,0,0,1.91488-4.279c1.57463-7.28437-1.91488-17.74856-1.91488-17.74856s27.29679-36.86241,25.7352-8.28761c-.62809,11.48052,2.80467,17.74857,7.13606,21.14211C1034.14853,401.73418,1042.5888,400.4038,1042.5888,400.4038Z"
                                    transform="translate(-43.05632 -143.875)" opacity="0.1" />
                                <ellipse cx="962.06082" cy="223.74844" rx="25.25834" ry="26.75955" opacity="0.1" />
                                <ellipse cx="962.06082" cy="222.87606" rx="25.25834" ry="26.75955" fill="#fbbebe" />
                                <path
                                    d="M1042.5888,400.4038l-12.21332,7.41523-20.06473,15.26665s-20.719-10.90475-15.48474-17.2295a9.80613,9.80613,0,0,0,1.91488-4.279c22.85636,19.54131,29.80047-.68482,30.95638-4.89406C1034.14853,401.73418,1042.5888,400.4038,1042.5888,400.4038Z"
                                    transform="translate(-43.05632 -143.875)" opacity="0.1" />
                                <path
                                    d="M993.97982,469.97609s-4.47968.51033-9.91895,1.025c-.21373-4.69341-.92477-10.00184-.7939-13.23836.2181-5.45237.2181-6.10666-1.52666-9.16s-1.52667-5.45238-1.30857-9.37809c.20936-3.76-.78516-4.51892-4.3226-10.52526-.15271-.26173-.30973-.53217-.47549-.81567-1.736-2.99226-2.23329-8.40539-2.2507-13.51752a.12429.12429,0,0,0,0-.05671c-.00437-1.217.01741-2.41651.061-3.55933.00437-.048.00437-.09594.00437-.14392.12651-3.45464.41-6.39892.55833-7.79908.60195-.57577.9727-.87674.9727-.87674l2.18095,1.74476,12.43141,21.15521Z"
                                    transform="translate(-43.05632 -143.875)" opacity="0.1" />
                                <path
                                    d="M1058.458,403.625c-.01744-.00873,2.634,1.99562,2.61653,1.99128-7.895-3.40664-12.37471,13.753-13.57423,19.15308-.22246.99453-.33151,1.58774-.33151,1.58774s.13961,1.44815.314,3.75122c.58889,7.71184,1.60083,24.98061-.75024,29.18112a6.51339,6.51339,0,0,0-.5627,1.40015c-.7415,2.55173-.65428,6.38583-.27038,10.11091a101.26535,101.26535,0,0,0,1.92356,11.38891s5.23428,13.30379,3.05333,22.02759a32.36341,32.36341,0,0,0-.63682,9.89278,58.504,58.504,0,0,0,1.38268,9.02477c.205.868.34461,1.36528.34461,1.36528q-4.93334,1.688-9.64415,2.80033a93.54989,93.54989,0,0,1-18.65148,2.52553c-29.4908.93782-49.12807-13.19475-53.76477-17.04628-.36639-.30534-.64119-.54524-.81568-.711-3.05333-2.83523,2.83524-5.01618,2.181-6.979s2.98358-6.32476,2.98358-6.32476-.80263-7.19713,2.4688-8.7238,5.67046-6.76094,7.51554-11.12284c1.84071-4.3619-.31841-15.70283-.10032-21.15521s.2181-6.10666-1.52666-9.16-1.52667-5.45238-1.30857-9.37809c.20936-3.76-.78516-4.51892-4.3226-10.52526-.15271-.26173-.30974-.53217-.47549-.81567-1.736-2.99226.96938-8.14664.952-13.25877.00436-.01746,1.00436.01744,1,0-.00437-1.217-2.04361-2.85718-2-4,.00436-.048-1-.952-1-1a32.39519,32.39519,0,0,0-.47863-7.69453c17.44759-8.94189,21.5914.37511,21.5914.37511,25.5171,21.96215,31.30972-5.48291,31.30972-5.48291,13.88827-4.74574,32.1559,8.039,33.1984,8.78051C1061.10943,405.625,1058.458,403.625,1058.458,403.625Z"
                                    transform="translate(-43.05632 -143.875)" fill="#55536e" />
                                <path
                                    d="M964.838,467.75249s-2.183,11.64719,4.13051,18.14557a7.1,7.1,0,0,0,8.21306,1.41145c2.09688-1.031,4.44194-2.56084,5.32212-4.52342C984.23841,478.918,978.796,460.01012,964.838,467.75249Z"
                                    transform="translate(-43.05632 -143.875)" fill="#fbbebe" />
                                <path
                                    d="M1046.7326,459.28944a6.51339,6.51339,0,0,0-.5627,1.40015l.99889-25.49964.314-5.08163C1048.07173,437.82016,1049.08367,455.08893,1046.7326,459.28944Z"
                                    transform="translate(-43.05632 -143.875)" opacity="0.1" />
                                <path
                                    d="M1050.87641,504.217a32.36341,32.36341,0,0,0-.63682,9.89278c-1.41326,2.56043-2.62151,4.73265-3.40667,6.1459-.615,1.09921-.9727,1.736-.9727,1.736a41.19653,41.19653,0,0,0-3.53749,5.30843,10.2626,10.2626,0,0,0-1.47869,4.31392c0,3.3543-3.38047,25.709-19.41045,20.14761-10.35078-3.58986-3.74249-14.59055,2.23766-21.936a82.14651,82.14651,0,0,1,6.37709-6.96158s1.84944-2.35543,3.62476-4.81989a29.27006,29.27006,0,0,0,3.13618-4.99438c.76333-2.18095,5.797-11.77713,5.797-11.77713s4.99872-10.5776,3.90825-15.48474c-.61063-2.748-.67612-9.53947-.615-14.98747a101.26535,101.26535,0,0,0,1.92356,11.38891S1053.05736,495.4932,1050.87641,504.217Z"
                                    transform="translate(-43.05632 -143.875)" opacity="0.1" />
                                <path
                                    d="M1061.12687,405.63808s7.85142,5.45238,7.63332,19.62855-2.83523,38.71185-2.83523,38.71185l-1.63571,15.26664s-2.07191,6.21571-1.74476,8.06952S1045.424,520.247,1045.424,520.247s-5.01618,6.27168-5.01618,9.62417-3.38047,25.70721-19.41045,20.14579,8.61475-28.89758,8.61475-28.89758,5.99761-7.63332,6.76094-9.81427S1042.172,499.528,1042.172,499.528s4.99684-10.5776,3.90637-15.48474-.43619-22.723-.43619-22.723l1.09047-27.875.43619-7.08809S1051.20355,402.80285,1061.12687,405.63808Z"
                                    transform="translate(-43.05632 -143.875)" fill="#fbbebe" />
                                <path
                                    d="M977.84205,457.6742l11.9011-4.323a5.63319,5.63319,0,0,1,5.79116,1.19929l15.79421,14.91676s.87238,2.39429-.29079,2.28762-18.17458-17.11807-18.17458-17.11807l-16.866,5.67046h0A2.80135,2.80135,0,0,1,977.84205,457.6742Z"
                                    transform="translate(-43.05632 -143.875)" fill="#00bf39" />
                                <polygon
                                    points="951.624 333.807 967.981 327.88 949.807 310.762 932.941 316.432 951.624 333.807"
                                    fill="#4b4b5b" />
                                <circle cx="927.05229" cy="381.35187" r="1.45397" fill="#f2f2f2" />
                                <circle cx="917.16527" cy="400.03392" r="1.45398" fill="#f2f2f2" />
                                <circle cx="996.14627" cy="406.82577" r="1.45396" fill="#f2f2f2" />
                                <circle cx="1004.36956" cy="433.21874" r="1.45398" fill="#f2f2f2" />
                                <circle cx="1001.16656" cy="425.79112" r="1.45398" fill="#f2f2f2" />
                                <circle cx="998.38703" cy="416.30841" r="1.45399" fill="#f2f2f2" />
                                <circle cx="921.78106" cy="390.19628" r="1.45398" fill="#f2f2f2" />
                                <circle cx="911.39801" cy="409.73367" r="1.45398" fill="#f2f2f2" />
                                <path
                                    d="M1061.07889,405.60318l-.00436.0131c-7.895-3.40664-12.37471,13.753-13.57423,19.15308l-12.40085-17.74856s-5.086,8.13929-21.51728,9.37808c-16.43124,1.23442-20.42675-12.50556-20.42675-12.50556-2.25512-.17448-12.46629,18.20656-16.1739,24.80612-.15271-.26173-.30974-.53217-.47549-.81567-1.736-2.99226-2.23329-8.40539-2.2507-13.51752a.12429.12429,0,0,0,0-.05671c.013-1.18645.03488-2.37724.061-3.55933.00437-.048.00437-.09594.00437-.14392.17885-4.84172.65865-8.67582.65865-8.67582,17.44759-8.94189,21.5914.37511,21.5914.37511,25.5171,21.96215,31.30972-5.48291,31.30972-5.48291C1041.76876,392.07693,1060.03639,404.86165,1061.07889,405.60318Z"
                                    transform="translate(-43.05632 -143.875)" fill="#00bf39" />
                                <path
                                    d="M964.29271,565.06548c.2181.54524,18.53807,21.70045,25.299,22.79093S964.29271,565.06548,964.29271,565.06548Z"
                                    transform="translate(-43.05632 -143.875)" opacity="0.1" />
                                <path
                                    d="M977.41051,532.61723s.186,14.91558-4.394,18.28407S977.41051,532.61723,977.41051,532.61723Z"
                                    transform="translate(-43.05632 -143.875)" opacity="0.1" />
                                <path
                                    d="M985.88411,537.91266s-.43619,17.55665,6.10666,21.70045C991.99077,559.61311,979.99555,547.945,985.88411,537.91266Z"
                                    transform="translate(-43.05632 -143.875)" opacity="0.1" />
                                <path
                                    d="M977.8146,628.09492s-12.21332,9.05094-15.59379,9.70523S977.8146,628.09492,977.8146,628.09492Z"
                                    transform="translate(-43.05632 -143.875)" opacity="0.1" />
                                <path
                                    d="M974.7656,351.1462c-2.63128,2.21724-1.68359,6.81984,1.00475,8.96755s6.36411,2.47745,9.80386,2.56616c6.62084.17075,13.3493-.21329,19.769,1.41531a16.02656,16.02656,0,0,1,7.58332,3.88476,17.06661,17.06661,0,0,1,3.94125,7.84032,42.93649,42.93649,0,0,1,1.3482,13.14281c-.15642,2.97384-.62194,5.9661-.19513,8.9133a28.50494,28.50494,0,0,0,2.78165,7.92946,329.46177,329.46177,0,0,0,16.84757,31.2541,5.81707,5.81707,0,0,0,2.195,2.40209c1.575.76033,3.42668-.0105,4.99967-.77495a37.53645,37.53645,0,0,1-1.54277-9.47076,4.23362,4.23362,0,0,0,4.475-1.56345,12.41834,12.41834,0,0,0,2.03861-4.58284l2.93454-10.11141c.90744-3.12675,2.25325-6.71538,5.37618-7.63591a19.4711,19.4711,0,0,0,3.29794-.76858,4.222,4.222,0,0,0,2.0369-4.15955,8.98407,8.98407,0,0,0-1.969-4.44041c-4.415-5.94025-11.77726-9.463-15.22357-16.013-2.95959-5.62487-2.44562-12.39362-1.46922-18.67415s2.3348-12.73739.83256-18.91328a25.05861,25.05861,0,0,0-9.72562-13.93155,39.2308,39.2308,0,0,0-15.90707-6.54037,15.882,15.882,0,0,0-8.04.05452c-1.983.67016-3.68141,1.99317-5.61693,2.79005-4.39726,1.81041-9.46615.72907-14.07978,1.88146a18.934,18.934,0,0,0-10.41411,7.52417,39.942,39.942,0,0,0-5.4535,11.86079c-.84961,2.773.51305,3.88235-1.96436,5.39029"
                                    transform="translate(-43.05632 -143.875)" opacity="0.1" />
                                <path
                                    d="M975.20179,350.27382c-2.63128,2.21724-1.68359,6.81984,1.00475,8.96755s6.36411,2.47745,9.80386,2.56616c6.62084.17075,13.3493-.21329,19.769,1.41531a16.02656,16.02656,0,0,1,7.58332,3.88476,17.0665,17.0665,0,0,1,3.94124,7.84032,42.93618,42.93618,0,0,1,1.34821,13.14281c-.15642,2.97384-.62194,5.9661-.19513,8.9133a28.50494,28.50494,0,0,0,2.78165,7.92946,329.46177,329.46177,0,0,0,16.84757,31.2541,5.81707,5.81707,0,0,0,2.195,2.40209c1.575.76033,3.42668-.0105,4.99967-.77495a37.53645,37.53645,0,0,1-1.54277-9.47076,4.23362,4.23362,0,0,0,4.475-1.56345,12.41834,12.41834,0,0,0,2.03861-4.58284l2.93454-10.11141c.90744-3.12675,2.25325-6.71538,5.37618-7.63591a19.4711,19.4711,0,0,0,3.29794-.76858,4.222,4.222,0,0,0,2.0369-4.15955,8.98407,8.98407,0,0,0-1.969-4.44041c-4.415-5.94025-11.77726-9.463-15.22357-16.013-2.95959-5.62487-2.44562-12.39362-1.46922-18.67415s2.3348-12.73739.83256-18.91328a25.05861,25.05861,0,0,0-9.72562-13.93155,39.2308,39.2308,0,0,0-15.90707-6.54037,15.88208,15.88208,0,0,0-8.04.05452c-1.983.67016-3.68141,1.99317-5.61693,2.79-4.39726,1.81041-9.46615.72907-14.07978,1.88146a18.934,18.934,0,0,0-10.41411,7.52417,39.942,39.942,0,0,0-5.4535,11.86079c-.84961,2.773.51305,3.88235-1.96436,5.39029"
                                    transform="translate(-43.05632 -143.875)" fill="#3f3d56" />
                                <path
                                    d="M972.47127,469.10371s19.95569-13.30379,25.62616-5.12523-17.55664,18.21093-17.55664,18.21093Z"
                                    transform="translate(-43.05632 -143.875)" fill="#fbbebe" />
                                <path
                                    d="M978.99355,405.88405c-.07325,1.2825-.31892,2.85493-1.03355,3.08753,1.86215-.04435,3.69756,1.32583,5.54553.9164,1.37608-.30488,2.67021-1.63957,3.50053-3.61023a1.68346,1.68346,0,0,1,.46864-.77027c.19416-.11365.40484-.00029.59884.11421a58.21646,58.21646,0,0,1,10.03848,7.57769c1.02241.95219,2.15681,1.98367,3.277,1.4863a25.02257,25.02257,0,0,0-2.14165-10.49378,13.41106,13.41106,0,0,0-5.7166-6.25441,17.50943,17.50943,0,0,0-6.72279-1.74968,6.73487,6.73487,0,0,1-2.89766-.72075c-.99141-.58255-4.1566-4.365-4.86728-1.61053-.34311,1.32981.09911,4.16256.10954,5.63366C979.1677,401.62451,979.11483,403.76042,978.99355,405.88405Z"
                                    transform="translate(-43.05632 -143.875)" fill="#3f3d56" />
                                <g opacity="0.1">
                                    <path
                                        d="M1042.14843,436.40872a5.81721,5.81721,0,0,1-2.195-2.40208,329.45713,329.45713,0,0,1-16.84756-31.25411,28.50429,28.50429,0,0,1-2.78167-7.92945c-.4268-2.94721.03871-5.93946.19514-8.9133a42.93606,42.93606,0,0,0-1.34821-13.1428,17.06665,17.06665,0,0,0-3.94123-7.84034,16.02653,16.02653,0,0,0-7.58332-3.88474c-6.41969-1.6286-13.14816-1.24457-19.769-1.41533-3.43977-.0887-7.11554-.41846-9.80387-2.56615a6.7741,6.7741,0,0,1-2.23992-7.1847,4.331,4.331,0,0,1-.55751.445c-2.56679,2.23375-1.61785,6.78717,1.05267,8.92063,2.68833,2.14769,6.3641,2.47744,9.80387,2.56615,6.62082.17076,13.34929-.21327,19.769,1.41533a16.02653,16.02653,0,0,1,7.58332,3.88474,17.06665,17.06665,0,0,1,3.94123,7.84034,42.93606,42.93606,0,0,1,1.34821,13.1428c-.15643,2.97383-.62194,5.96609-.19514,8.9133a28.5044,28.5044,0,0,0,2.78167,7.92945,329.45713,329.45713,0,0,0,16.84756,31.25411,5.81721,5.81721,0,0,0,2.195,2.40208c1.575.76035,3.42669-.01049,4.99967-.77494-.13077-.43853-.25118-.88-.36577-1.32308A3.98394,3.98394,0,0,1,1042.14843,436.40872Z"
                                        transform="translate(-43.05632 -143.875)" />
                                    <path
                                        d="M974.98919,350.51078c.10561-.0643.195-.127.28715-.19.01712-.0149.03049-.03221.04792-.0469Z"
                                        transform="translate(-43.05632 -143.875)" />
                                    <path
                                        d="M977.0211,348.1398c-.09214.063-.18154.12574-.28715.19l.33507-.23694c-.01743.01469-.0308.032-.04792.04691,2.06887-1.41469.86384-2.54547,1.67722-5.20026a40.34835,40.34835,0,0,1,5.32256-11.66747,23.1343,23.1343,0,0,0-1.61383,1.98764,39.94176,39.94176,0,0,0-5.45349,11.86078,18.56117,18.56117,0,0,0-.42682,3.53473A4.17039,4.17039,0,0,1,977.0211,348.1398Z"
                                        transform="translate(-43.05632 -143.875)" />
                                    <path
                                        d="M1063.72766,401.50083a19.47132,19.47132,0,0,1-3.29794.76858,6.36053,6.36053,0,0,0-3.64851,3.22129,5.23728,5.23728,0,0,1,1.90375-1.04035,19.47108,19.47108,0,0,0,3.29794-.76857,3.71942,3.71942,0,0,0,1.80756-2.21615C1063.76916,401.47687,1063.7493,401.49005,1063.72766,401.50083Z"
                                        transform="translate(-43.05632 -143.875)" />
                                    <path
                                        d="M1045.60533,426.163q.03762,1.13406.14592,2.26425a4.91225,4.91225,0,0,0,2.58437-1.64676,7.28053,7.28053,0,0,0,.86679-1.34871A3.88793,3.88793,0,0,1,1045.60533,426.163Z"
                                        transform="translate(-43.05632 -143.875)" />
                                </g>
                                <path d="M967.64432,466.50791s5.26224-2.13014,8.35282-1.657"
                                    transform="translate(-43.05632 -143.875)" opacity="0.1" />
                                <path
                                    d="M982.8158,490.477s2.67087,4.14381,16.17338,5.88857S982.8158,490.477,982.8158,490.477Z"
                                    transform="translate(-43.05632 -143.875)" opacity="0.1" />
                                <path
                                    d="M977.96,505.96176s12.14489,10.68665,31.6259,6.76094S977.96,505.96176,977.96,505.96176Z"
                                    transform="translate(-43.05632 -143.875)" opacity="0.1" />
                                <line x1="880.40168" y1="611.75" x2="1080.40168" y2="580.75" fill="none"
                                    stroke="#3f3d56" stroke-miterlimit="10" />
                                <path
                                    d="M224.36673,418.99808c0,26.13213,17.16823,48.71308,42.0635,59.41566l-35.6018-86.28266a59.38557,59.38557,0,0,0-6.4617,26.867Zm125.01888-3.33137c0-8.15883-3.31321-13.80909-6.15494-18.207-3.78329-5.438-7.328-10.04309-7.328-15.48106,0-6.06857,5.202-11.71757,12.53142-11.71757.3309,0,.64476.03643.96855.05276a79.973,79.973,0,0,0-50.40123-17.33015c-26.074,0-49.01373,11.83314-62.3589,29.7562,1.751.04648,3.40268.07788,4.803.07788,7.80658,0,19.89066-.83786,19.89066-.83786,4.0233-.20978,4.49763,5.01715.48,5.438,0,0-4.0446.42082-8.54364.62934l27.17888,71.50757L296.786,416.2257,285.15637,388.046c-4.019-.20853-7.82646-.62934-7.82646-.62934-4.02188-.20853-3.55039-5.64775.47149-5.438,0,0,12.3255.83787,19.65918.83787,7.80516,0,19.89065-.83787,19.89065-.83787,4.02613-.20978,4.499,5.01715.47859,5.438,0,0-4.05169.42081-8.54222.62934l26.97154,70.96617,7.443-22.00436c3.228-9.13111,5.68345-15.68958,5.68345-21.3411Z"
                                    transform="translate(-43.05632 -143.875)" fill="#00bf39" />
                                <path
                                    d="M300.31082,424.7727l-22.393,57.55528a83.94664,83.94664,0,0,0,45.8681-1.05142,5.38774,5.38774,0,0,1-.53114-.90947ZM364.49036,387.325a44.91214,44.91214,0,0,1,.50273,6.78835c0,6.69916-1.41448,14.22991-5.67493,23.64617l-22.79772,58.3027c22.18847-11.445,37.11286-32.70821,37.11286-57.06411a59.7826,59.7826,0,0,0-9.143-31.67312Z"
                                    transform="translate(-43.05632 -143.875)" fill="#00bf39" />
                            </svg>
                            <div class="card__body">
                                <h4 class="card__head">CMS開発</h4>
                                <p class="card__content">主に静的ページから動的ページにするWPへ組み込む開発が我々の特徴</p>
                            </div>
                        </div>
                    </div>

                </div>
            </div>
        </main>
    </section>
    <section class="team">
        <h2 class="heading_primary">member</h2>
        <div class="container team_detail">
            <div class="row">
                <div class="col-12 col-sm-6 col-lg-4 ">
                    <div class="card1 animate__animated animate__fadeInUp wow">
                        <div class="imgBx">
                            <img src="./img/sher.jpg" class="img-fluid" alt="" />
                        </div>
                        <div class="content">
                            <div class="contentBx">
                                <h3>ケーシー・セール・バハドウール<br /><span>事業最高責任者</span></h3>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-sm-6 col-lg-4">
                    <div class="card1 animate__animated animate__fadeInUp wow" data-wow-delay="250ms">
                        <div class="imgBx">
                            <img src="./img/hebin.jpg" class="img-fluid" alt="" />
                        </div>
                        <div class="content">
                            <div class="contentBx">
                                <h3>へべン<br /><span>CEO・プロジェクトマネージャー</span></h3>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-sm-6 col-lg-4">
                    <div class="card1 animate__animated animate__fadeInUp wow" data-wow-delay="500ms">
                        <div class="imgBx">
                            <img src="./img/vivek.jpg" class="img-fluid" alt="" />
                        </div>
                        <div class="content">
                            <div class="contentBx">
                                <h3>ビべク <br /><span>CTO・フルスタック</span></h3>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-sm-6 col-lg-4">
                    <div class="card1 animate__animated animate__fadeInUp wow" data-wow-delay="750ms">
                        <div class="imgBx">
                            <img src="./img/nishan.jpg" class="img-fluid" alt="" />
                        </div>
                        <div class="content">
                            <div class="contentBx">
                                <h3>二シャン<br /><span>COO・フルスタック　 </span></h3>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <section class="contact section-ctct ctct backcolor">
        <div class="container">
            <div class="hdng" id="contact">
                <h2 class="heading_primary">Contact</h2>
            </div>
            <div class="ctct-wrapper">
                <div class="ctct-row">
                    <div class="ctct-col ctct-col-lft animate__animated animate__fadeInLeft wow">
                        <h3 class="prgh-lg ">お問い合わせはこちらから</h3>
                        <div class="ctct-inqry inqry">
                            <form action="<?php echo $_SERVER['PHP_SELF'];?>" class="inqry-form contact-form" method="post" id="form">
                                <div class="row">
                                    <div class="col-md-6 mb-4">
                                        <input type="text" class="form-control inqry-input" id="firstName" name="fname"
                                            placeholder="氏">
                                    </div>
                                    <div class="col-md-6 mb-4">
                                        <input type="text" class="form-control inqry-input" id="lastName" name="lname"
                                            placeholder="名">
                                    </div>
                                    <div class="col-md-6 mb-4">
                                        <input type="text" class="form-control inqry-input" id="phoneNumber" name="pnumber"
                                            placeholder="電話番号">
                                    </div>
                                    <div class="col-md-6 mb-4">
                                        <input type="email" class="form-control inqry-input" id="emailAddress" name="email"
                                            placeholder="メール">
                                    </div>
                                    <div class="col-12 mb-4">
                                        <textarea name="information" id="detailedInfo" rows="5"
                                            class="form-control inqry-textarea inqry-input" name="detail"
                                            placeholder="自由記入欄"></textarea>
                                    </div>
                                </div>
                                <div class="inqry-btn-wrpr">
                                    <button type="submit" class="btn-inqry">
                                        <svg width="21" height="18" viewBox="0 0 21 18" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path d="M0.00999999 18L21 9L0.00999999 0L0 7L15 9L0 11L0.00999999 18Z"
                                                fill="white" />
                                        </svg>
                                        <span class="btn-inqry-txt">送信する</span>
                                    </button>
                                </div>
                            </form>
                        </div>
                    </div>
                    <div class="ctct-col ctct-col-rgt">
                        <h3 class="wht">連絡先情報</h3>
                        <address class="ctct-info">
                            <i class="fas fa-map-marker-alt"></i>chandragiri,kathmandu,nepal
                        </address>
                        <a href="tel:9861692429" class="ctct-info">
                            <i class="fas fa-mobile-alt"></i>+977-9861-69-2429
                        </a>
                        <a href="mailto:kchebin2004@gmail.com" class="ctct-info">
                            <i class="fas fa-envelope"></i>kchebin2004@gmail.com
                        </a>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <section class="company_info" id="company">
        <div class="container company">
            <h2 class="heading_primary">company</h2>
            <table class="animate__animated animate__fadeInRight wow" data-wow-delay="200ms">
                <tbody>
                    <tr>
                        <td>社名</td>
                        <td>
                            HVNcreations pvt.ltd
                        </td>
                    </tr>
                    <tr>
                        <td>事業内容</td>
                        <td>
                            オフショアソフトウェア開発,ホームページ制作及びモバイルアプリ開発サービスの提供。
                        </td>
                    </tr>
                    <tr>
                        <td>設⽴</td>
                        <td>2019年</td>
                    </tr>
                    <tr>
                        <td>本社</td>
                        <td>カトマンズ、ネパール。名古屋仮拠点</td>
                    </tr>
                    <tr>
                        <td>事業最高責任者</td>
                        <td>ケーシー・セール・バハドウール</td>
                    </tr>
                    <tr>
                        <td>代表者</td>
                        <td>カタリ へべン</td>
                    </tr>
                    <tr>
                        <td>資本⾦</td>
                        <td>200万円</td>
                    </tr>
                    <tr>
                        <td>従業員</td>
                        <td>10名</td>
                    </tr>
                    <tr>
                        <td>電話番号</td>
                        <td><a href="tel:>01-5905279">+977-9861-69-2429</a></td>
                    </tr>
                    <tr>
                        <td class="nobod">メール</td>
                        <td class="nobod">
                            <a href="mailto:kchebin2004@gmail.com"></a>kchebin2004@gmail.com
                        </td>
                    </tr>
                </tbody>
            </table>
        </div>
        <div class="sakura">
            <img src="./img/sakura.png" class="img-fluid" alt="">
        </div>
    </section>
    <section class="foot">
        <footer id="site-footer">

            <section class="horizontal-footer-section" id="footer-top-section">
                <div id="footer-logo">
                    <picture>
                        <img src="./img/hvn11.png" alt="footer logo" role="presentation">
                    </picture>
                </div>
                <div id="footer-top-menu-container" role="menubar">
                    <ul id="footer-top-menu" role="menu">
                        <li class="footer-top-menu-item" role="menuitem">
                            <a href="#" class="footer-top-menu-link">IT サービス</a>
                        </li>
                        <li class="footer-top-menu-item footer-top-menu-item-active" role="menuitem">
                            <a href="#" class="footer-top-menu-link">Web開発</a>
                        </li>
                        <li class="footer-top-menu-item" role="menuitem">
                            <a href="#" class="footer-top-menu-link">SEO</a>
                        </li>
                        <li class="footer-top-menu-item" role="menuitem">
                            <a href="#" class="footer-top-menu-link">UI/UX</a>
                        </li>
                    </ul>
                </div>
                <div id="footer-buttons-container">
                    <a href="#" class="footer-button" role="button">問い合わせる</a>
                </div>
            </section>
            <section class="horizontal-footer-section" id="footer-middle-section">
                <div id="footer-about" class="footer-columns footer-columns-large">
                    <address>
                        <p><img src="https://img.icons8.com/ios-filled/14/eeeeee/marker.png">44600 カトマンズ　ネパール</p>
                        <p><img src="https://img.icons8.com/ios-filled/14/eeeeee/phone.png">9861692429</p>
                        <p><img src="https://img.icons8.com/ios-filled/14/eeeeee/mail.png">kchebin2004@gmail.com</p>
                        <p><img src="https://img.icons8.com/ios-filled/14/eeeeee/clock.png">いつでも承ります！お気軽にご連絡ください❣</p>
                    </address>
                </div>
                <div class="footer-columns">
                    <ul class="footer-column-menu" role="menu">
                        <li class="footer-column-menu-item" role="menuitem">
                            <a href="#" class="footer-column-menu-item-link">私たちについて</a>
                        </li>
                        <li class="footer-column-menu-item" role="menuitem">
                            <a href="#" class="footer-column-menu-item-link">サービス</a>
                        </li>
                    </ul>
                </div>
                <div class="footer-columns">
                    <ul class="footer-column-menu" role="menu">
                        <li class="footer-column-menu-item" role="menuitem">
                            <a href="#" class="footer-column-menu-item-link">メンバー </a>
                        </li>
                        <li class="footer-column-menu-item" role="menuitem">
                            <a href="#" class="footer-column-menu-item-link">会社概要</a>
                        </li>
                    </ul>
                </div>
            </section>
            <div class="copyright-text">
                <p> HVNcreations. 2022. All rights reserved.</p>
            </div>
    </section>
    </footer>
    <div class="fixed_content">
        <ul class="sci">
            <li>
                <a href="#"><i class="fab fa-facebook-f"></i></a>
            </li>
            <li>
                <a href="#"><i class="fab fa-twitter"></i></a>
            </li>
            <li>
                <a href="#"><i class="fab fa-linkedin-in"></i></a>
            </li>
            <li>
                <a href="#"><i class="fab fa-instagram"></i></a>
            </li>
        </ul>
    </div>
    </section>
</body>
<!-- Swiper JS -->
<script src="https://cdnjs.cloudflare.com/ajax/libs/wow/1.1.2/wow.min.js"
    integrity="sha512-Eak/29OTpb36LLo2r47IpVzPBLXnAMPAVypbSZiZ4Qkf8p/7S/XRG5xp7OKWPPYfJT6metI+IORkR5G8F900+g=="
    crossorigin="anonymous" referrerpolicy="no-referrer"></script>

<script src="app.js"></script>
</body>

</html>

